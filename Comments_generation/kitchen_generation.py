...
def build_prompt(cluster_id, features):
    prompt = f"""
The following is a user profile from a Reddit discussion about cooking and kitchen experiences.

Profile C{cluster_id}
Traits: {', '.join(features)}

Based on this profile, generate a sample Reddit post. Use a tone and content that fits the listed traits. 
Be authentic, not exaggerated. Focus on food, personal experiences, cooking ideas, or curiosities.
...
def main():
    ...
    input_files = [
        "dataset_iniziale_kitchen/kitchen_vettoriNONpuliti_centroidi.json",
        "dataset_iniziale_kitchen/kitchen_vettoriNONpuliti_medoidi.json",
        "dataset_iniziale_kitchen/kitchen_vettoripuliti_centroidi.json",
        "dataset_iniziale_kitchen/kitchen_vettoripuliti_medoidi.json"
    ]
    ...
    for file_index, input_path in enumerate(input_files):
        ...
        for i, cluster in enumerate(clusters):
            ...
            for j in range(n):
                ...
                conversable = autogen.ConversableAgent(
                    name=f"CommentGenerator_file{file_index}_cluster{cluster_id}_rep{j}",
                    llm_config=llm_config_gemma,
        ...
        output_path = os.path.join("risultati_kitchen", output_filename)
